...
class TestEuropeanCharmThreadSafety:
    def test_charm_does_not_mutate_global_evaluation_date(self):
        ...
        # Align global eval date so the engine prices correctly; then verify
        # price_european does not mutate it.
        ...
        try:
            ...
            assert ql.Settings.instance().evaluationDate == ql.Date(20, 4, 2026)
        finally:
    ...
    def test_charm_near_max_date_does_not_crash(self):
        ...
        original = ql.Settings.instance().evaluationDate
        ql.Settings.instance().evaluationDate = ql.Date(30, 12, 2199)
        try:
            result = price_european(
                s=100.0,
                k=100.0,
            ...
            assert result.charm == 0.0
            assert ql.Settings.instance().evaluationDate == ql.Date(30, 12, 2199)
        finally:
            ql.Settings.instance().evaluationDate = original
